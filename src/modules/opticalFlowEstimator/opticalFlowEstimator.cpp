...
void OpticalFlowEstimator::operator()(VideoStream& videoStream, cv::Mat& frame){
    cv::cvtColor(frame, frame0, cv::COLOR_RGB2GRAY);

    if (frame1.empty()) {
        frame0.copyTo(frame1);
        BOOST_LOG_TRIVIAL(info) << "Outer loop";
    }
        ...
        hsv.convertTo(hsv8, CV_8U, 255.0);
            ...
            // Save the results
        cv::cvtColor(hsv8, optical_flow_image, cv::COLOR_HSV2RGB);
        
        videoStream.opticalFlowQueue.enqueueFrame(optical_flow_image.clone());

        BOOST_LOG_TRIVIAL(info) << "Estimated Optical Flow frame : " << count << " from stream ID : " << videoStream.streamID;

        count ++;
    }   
}

void OpticalFlowEstimator::compute(VideoStream& videoStream, cv::Mat frame){
    cv::cvtColor(frame, frame0, cv::COLOR_RGB2GRAY);
